...
--
-- Transformer-AI-ERP is row-level multi-tenant in a single Supabase project (every
-- tenant company shares one database, distinguished by `company_id`, rather than one
-- Supabase project per customer). Every operational table gets a `company_id` column
-- and is scoped by `current_company_id()` in its RLS policies, generated by a per-table
-- do-block that emits matching `has_permission()`-gated select/insert/update/delete
-- policies:
--
--   do $$
--   declare tbl text;
