...
import { salesOrderSchema, type SalesOrderFormInput, type SalesOrderFormValues } from '@/features/sales/schemas/sales-schemas'
import { useCustomers } from '@/features/sales/hooks/use-customers'
import { useCreateSalesOrder } from '@/features/sales/hooks/use-sales-orders'
...
export function SalesOrderFormDrawer({ open, onOpenChange }: SalesOrderFormDrawerProps) {
  ...
  const onSubmit = (values: SalesOrderFormValues) => {
    createSalesOrder.mutate(values, { onSuccess: () => onOpenChange(false) })
  }
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="data-[side=right]:w-full data-[side=right]:sm:max-w-none">
        <SheetHeader className="mx-auto w-full max-w-4xl">
          ...
          <SheetDescription>Create a direct sales order for a customer.</SheetDescription>
        </SheetHeader>

        <ScrollArea className="min-h-0 flex-1 px-4">
          <form id="so-form" onSubmit={handleSubmit(onSubmit)} className="mx-auto w-full max-w-4xl space-y-6 pb-6">
              ...
              <div className="space-y-1.5">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select value={field.value} onValueChange={(v) => field.onChange(v ?? '')}>
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Select warehouse">{warehouses?.find((w) => w.id === field.value)?.name ?? ''}</SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        {warehouses?.map((w) => (
                ...
                <Table className="table-fixed">
                  ...
                  <TableBody>
                    {fields.map((field, index) => {
                      ...
                      const insufficientStock = product ? qty > product.total_stock : false
                          ...
                          <TableCell>
                            ...
                            {insufficientStock && (
                              <p className="mt-1 flex items-center gap-1 text-[11px] text-destructive">
                                <AlertTriangle className="size-3" /> Only {product?.total_stock} in stock
                              </p>
                            )}
                          </TableCell>
        ...
        <div className="border-t border-border px-4 py-3">
          <div className="mx-auto flex w-full max-w-4xl justify-end gap-2">
            ...
            <Button type="submit" form="so-form" disabled={createSalesOrder.isPending}>
